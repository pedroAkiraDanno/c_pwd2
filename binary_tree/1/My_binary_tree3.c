...
void generateRandomDataToFile(long numData, const char* filename) {
    FILE* file = fopen(filename, "w");
    if (file != NULL) {
        srand(time(NULL));
        for (long i = 0; i < numData; i++) {
            //fprintf(file, "%ld\n", rand() % 10000); // Generates random numbers between 0 and 99999
            fprintf(file, "%ld\n", (long int)(rand() % 100000));
        }
        fclose(file);
    } else {
...
int main() {
    ...
    generateRandomDataToFile(MAX_DATA, "random_data.txt");

    // Read data from file and insert into BST
    FILE* 
     = fopen("random_data.txt", "r");
    if (file != NULL) {
        int data;
