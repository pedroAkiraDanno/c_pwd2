...
#define MAX_RANDOM 500000000   
//#define MAX_SEED   900000
#define MAX_SEED   9999999                   
...
int main() {
    ...
    if(is_sorted(uniqueData, newSize)) {
    ...
    printDefault(2);    
    writeToFile(uniqueData, newSize, "data.bin");
    ...
    printDefault(3);
    readFromFile(uniqueData, newSize, "data.bin");



    printDefault(7);
    // Write binary to text
    writeBinToTxt("data.bin", "unsorted_data.txt");




    printDefault(4);
    quicksort(uniqueData, 0, newSize - 1);
    printDefault(2);    
    writeToFile(uniqueData, newSize, "sorted_data.bin");
    ...
    printDefault(5);
    long target = 420000;
    long result = binarySearch(uniqueData, newSize, target);
    ...
    while (searchAgain) {
         ...
         long result = binarySearch(uniqueData, newSize, number);
