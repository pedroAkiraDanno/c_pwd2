...
void generateRandomPerson(Person* person) {
    ...
    strcpy(person->name, names[rand() % 7]);
...
void insertRandomData(Person* people, int* n) {
    (*n)++;
    generateRandomPerson(&people[*n - 1]);
    sortByNames(people, *n);
}

int main() {
    srand(time(NULL));
    int n = 5; // Change this to the desired initial number of random data
    ...
    do {
        ...
        printf("4. Insert more random data\n");
        printf("5. Exit\n");
        ...
        switch (choice) {
            ...
            case 4:
                ...
                writeToFile(people, n, "sorted_data.txt");
                printf("Random data inserted and written to file.\n");
                break;
            case 5:
                printf("Exiting...\n");
                break;
